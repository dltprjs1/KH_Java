 ...
 *  변수는 여러 개의 값을 저장할 수 없고, 오직 하나의 값만을 저장 할 수 있음
 *  필요한 데이터를 저장할 곳을 미리 선언해 두고, 나중에 계속해서 사용하기 위해 변수를 만듬
 *  데이터를 저장하기 위해 생성되는 메모리 공간은 목적에 따라 크기와 특징이 다른데, 이를 자료형이라고 함. 즉, 메모리 공간의 자료형에 따라 저장할 수 있는 값의 종류와 크기가 결정이 됨.
 *  
 */
...
/*
 * 변수명(식별자) 만드는 작성 규칙
 * -변수명, 메서드명, 클래스명 등의 이름을 지정할 떄 사용함.
 * 1. 예약어(키워드 - 문법적으로 사용되는 단어) 사용불가
 * 2. 특수문자는 사용불가 ( _ and $ 제외)
