 ...
 * - 실세계에서의 상속은 상위 객체의 속성이 하위 객체에 물려줘서
 * 	 하위 객체가 상위 객체의 속성을 모두 가지는 개념
 * - 자바에서의 상속은 자식클래스가 부모클래스의 속성을 물려 받고 
 *   기능을 추가하여 확장(extention) 하는 개념
 * - 상속은 슈퍼클래스의 필드와 메서드를 물려 받아서 코드를 재사용함으로써, 코드 작성에
 *   드는 시간과 비용을 줄일 수 있다
 * - 기존에 이미 잘 만들어진 클래스를 재사용하여 새로운 클래스를 생성하는 자바 문법 ***중요***
 *   즉, 특정 클래스를 만들 떄 기존의 클래스의 데이터(속성)와 기능(메서드)을 그대로 물려 받아서 중복적인 코드를
 *   없애준다.(클래스의 재사용, 코드의 중복 제거)
 * - 자식과 부모의 관계로 형성이 되어 있다.
 ...
 * - 상속은 단일 상속만 가능!!!
 *   ==> 즉, 상속을 받을 수 있는 클래스는 하나이다. 그러나 부모클래스는 여러 자식 클래스를 가질 수 있다.
 * - 상속의 장점은 클래스의 수정을 최소화시킬 수 있다는 장점이 있다
 * 	 또한 유지보수의 시간을 최소화시켜 준다는 장점이 있다.
 * 
 * 		형식)
 ...
 * 
 * - 주의사항) 부모클래스는 반드시 기존에 미리 정의되어 있어야 한다.
 */
